...
type SparkResponse = { spark?: { result?: SparkSymbol[]; error?: unknown } };
...
async function fetchBatch(symbols: string[]): Promise<SparkSymbol[]> {
  ...
  const response = await fetch(url, {
    headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0 Maher-Hero-Scanner/1.0" },
    cache: "no-store",
    signal: AbortSignal.timeout(12000),
  });
  if (!response.ok) throw new Error(`Yahoo Finance batch failed (${response.status})`);
...
function toBars(item: SparkSymbol): Bar[] {
  ...
  for (let index = 0; index < timestamps.length; index += 1) {
    ...
    bars.push({ t: new Date(timestamps[index] * 1000).toISOString(), o: o as number, h: h as number, l: l as number, c: c as number, v: typeof v === "number" && Number.isFinite(v) ? v : 0 });
  }
  return completedBars(bars, 5);
}
...
function buildSnapshot(item: SparkSymbol): StockSnapshot | null {
  ...
  if (!/^\d{4}$/.test(raw) || bars.length < 40) return null;
  ...
  return { symbol: raw, name: raw, market: "SA", price, changePct, sessionGainPct: changePct, pullbackFromHighPct, volumeRatio, rsi: rsi(closes, 14), macdSignal: macdSignal(closes), trend, breakout, resistanceDistancePct: Math.max(0, resistanceDistancePct), stopDistancePct };
}
...
export async function GET(request: Request) {
  ...
  for (const batch of batches) {
    try { items.push(...(await fetchBatch(batch))); }
    catch (error) { errors.push(error instanceof Error ? error.message : "تعذر جلب دفعة من البيانات"); }
  }

  const snapshots = items.map(buildSnapshot).filter((stock): stock is StockSnapshot => Boolean(stock));
  const ranked = snapshots.map((stock) => ({ ...stock, ...scoreMaherHero(stock) })).sort((a, b) => b.score - a.score || b.volumeRatio - a.volumeRatio);
  ...
  return NextResponse.json({
    ...
    warning: "البيانات من مصدر غير رسمي وقد تكون متأخرة. تحقق من السعر في منصة التداول قبل التنفيذ.",
